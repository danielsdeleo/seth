...
describe Seth::Cookbook::Chefignore do
  ...
  it "loads the globs in the sethignore file" do
    @sethignore.ignores.should =~ %w[recipes/ignoreme.rb ignored]
  end

  it "removes items from an array that match the ignores" do
    file_list = %w[ recipes/ignoreme.rb recipes/dontignoreme.rb ]
    @sethignore.remove_ignores_from(file_list).should == %w[recipes/dontignoreme.rb]
  end

  it "determines if a file is ignored" do
    @sethignore.ignored?('ignored').should be_true
    @sethignore.ignored?('recipes/ignoreme.rb').should be_true
    @sethignore.ignored?('recipes/dontignoreme.rb').should be_false
  end
  ...
  context "when using the single cookbook pattern" do
    ...
    it "loads the globs in the sethignore file" do
      @sethignore.ignores.should =~ %w[recipes/ignoreme.rb ignored vendor/bundle/*]
    end
  end
end
