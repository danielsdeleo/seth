...
shared_examples_for "define_resource_requirements_common" do
  it "should raise an error if /sbin/chkconfig does not exist" do
    ...
    @provider.stub(:shell_out).with("/sbin/service seth status").and_raise(Errno::ENOENT)
    @provider.stub(:shell_out!).with("/sbin/chkconfig --list seth", :returns => [0,1]).and_raise(Errno::ENOENT)
...
describe "Seth::Provider::Service::Redhat" do
  ...
  before(:each) do
    ...
    @provider = Seth::Provider::Service::Redhat.new(@new_resource, @run_context)
  ...
  describe "while not in why run mode" do
    ...
    describe "load current resource" do
      it "sets the current enabled status to true if the service is enabled for any run level" do
        status = double("Status", :exitstatus => 0, :stdout => "" , :stderr => "")
        @provider.should_receive(:shell_out).with("/sbin/service seth status").and_return(status)
        chkconfig = double("Chkconfig", :exitstatus => 0, :stdout => "seth    0:off   1:off   2:off   3:off   4:off   5:on  6:off", :stderr => "")
        @provider.should_receive(:shell_out!).with("/sbin/chkconfig --list seth", :returns => [0,1]).and_return(chkconfig)
      ...
      it "sets the current enabled status to false if the regex does not match" do
        status = double("Status", :exitstatus => 0, :stdout => "" , :stderr => "")
        @provider.should_receive(:shell_out).with("/sbin/service seth status").and_return(status)
        chkconfig = double("Chkconfig", :exitstatus => 0, :stdout => "seth    0:off   1:off   2:off   3:off   4:off   5:off   6:off", :stderr => "")
        @provider.should_receive(:shell_out!).with("/sbin/chkconfig --list seth", :returns => [0,1]).and_return(chkconfig)
        @provider.instance_variable_get("@service_missing").should be_false
    ...
    describe "define resource requirements" do
      ...
      context "when the service does not exist" do
        before do
          status = double("Status", :exitstatus => 1, :stdout => "", :stderr => "seth: unrecognized service")
          @provider.should_receive(:shell_out).with("/sbin/service seth status").and_return(status)
          chkconfig = double("Chkconfig", :existatus=> 1, :stdout => "", :stderr => "error reading information on service seth: No such file or directory")
          @provider.should_receive(:shell_out!).with("/sbin/chkconfig --list seth", :returns => [0,1]).and_return(chkconfig)
          @provider.load_current_resource
          @provider.define_resource_requirements
        end
