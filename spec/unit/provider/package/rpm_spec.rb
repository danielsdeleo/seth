...
describe Seth::Provider::Package::Rpm do
  before(:each) do
    @node = Seth::Node.new
    @events = Seth::EventDispatch::Dispatcher.new
    @run_context = Seth::RunContext.new(@node, {}, @events)

    @new_resource = Seth::Resource::Package.new("ImageMagick-c++")
    @new_resource.source "/tmp/ImageMagick-c++-6.5.4.7-7.el6_5.x86_64.rpm"

    @provider = Seth::Provider::Package::Rpm.new(@new_resource, @run_context)
  ...
  describe "when determining the current state of the package" do
    ...
    it "should raise an exception if a source is supplied but not found" do
      ...
      lambda { @provider.run_action(:any) }.should raise_error(Seth::Exceptions::Package)
    end
  ...
  describe "after the current resource is loaded" do
    before do
      @current_resource = Seth::Resource::Package.new("ImageMagick-c++")
    ...
    describe "when installing or upgrading" do
      ...
      it "should install from a path when the package is a path and the source is nil" do
        @new_resource = Seth::Resource::Package.new("/tmp/ImageMagick-c++-6.5.4.7-7.el6_5.x86_64.rpm")
        @provider = Seth::Provider::Package::Rpm.new(@new_resource, @run_context)
