...
describe Seth::Formatters::ErrorInspectors::ResourceFailureInspector do
  ...
  before do
    @description = Seth::Formatters::ErrorDescription.new("Error Converging Resource:")
    @stdout = StringIO.new
    @outputter = Seth::Formatters::IndentableOutputStream.new(@stdout, STDERR)
    #@outputter = Seth::Formatters::IndentableOutputStream.new(STDOUT, STDERR)

    Seth::Config.stub(:cookbook_path).and_return([ "/var/seth/cache" ])
  end
  ...
  describe "when explaining an error converging a resource" do
    ...
    it "filters seth core code from the backtrace" do
    ...
    describe "and the error is a template error" do
      before do
        @description = Seth::Formatters::ErrorDescription.new("Error Converging Resource:")
        @template_class = Class.new { include Seth::Mixin::Template }
        ...
        @inspector = Seth::Formatters::ErrorInspectors::ResourceFailureInspector.new(@resource, :create, @error)
        @inspector.add_explanation(@description)
      end
