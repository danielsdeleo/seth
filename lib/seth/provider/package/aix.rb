...
class Seth
  class Provider
    class Package
      class Aix < Seth::Provider::Package
        ...
        def load_current_resource
          ...
          status = popen4("lslpp -lcq #{@current_resource.package_name}") do |pid, stdin, stdout, stderr|
            stdout.each do |line|
              ...
              when /#{@current_resource.package_name}/
                fields = line.split(":")
                Seth::Log.debug("#{@new_resource} version #{fields[2]} is already installed")
          ...
          unless status.exitstatus == 0 || status.exitstatus == 1
            raise Seth::Exceptions::Package, "lslpp failed - #{status.inspect}!"
          end
        ...
        def candidate_version
          ...
          unless status.exitstatus == 0
            raise Seth::Exceptions::Package, "installp -L -d #{@new_resource.source} - #{status.inspect}!"
          end
          @candidate_version
        end
