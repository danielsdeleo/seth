...
class Seth
  class Config
    ...
    # Config file to load (client.rb, ceth.rb, etc. defaults set differently in ceth, seth-client, etc.)
    ...
    # No config file (client.rb / ceth.rb / etc.) will be loaded outside this path.
    ...
    # seth-solo uses these directories for input data.  ceth commands
    # that upload or download files (such as ceth upload, ceth role from file,
    # etc.) work.
    ...
    # An array of paths to search for ceth exec scripts if they aren't in the current directory
    default :script_path, []
    ...
    # Syntax Check Cache. ceth keeps track of files that is has already syntax
    # checked by storing files in this directory. `syntax_check_cache_path` is
    # the new (and preferred) configuration setting. If not set, ceth will
    ...
    # ceth configuration data
    config_context :ceth do
